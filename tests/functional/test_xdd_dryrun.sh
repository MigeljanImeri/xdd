...
# Blocksize 4m, Bytes 1MB, Targets 1, QueueDepth 4, Passes 1
data_file="${test_dir}/test"

# write a file
# Old: reqsize 4096
"${XDDTEST_XDD_EXE}" -op write -blocksize 4m -mbytes 1 -targets 1 "${data_file}" -qd 4 -passes 1 -datapattern random

# now run dryrun
sleep_seconds=2
"${XDDTEST_XDD_EXE}" -op read  -blocksize 1 -numreqs 1 -targets 1 "${data_file}" -dryrun &
pid=$!
